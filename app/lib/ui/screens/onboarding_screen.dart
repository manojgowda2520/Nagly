...
enum _Step { weight, activity, hours, building, goal, mode, medication, relationship, variant, firstGlass, permission }
...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  Future<void> _finish({required bool askPermission}) async {
    ...
    if (_mode == CareMode.medication && _medName.text.trim().isNotEmpty) {
      await c.addMedication(_medName.text.trim(), _medTime.hour, _medTime.minute);
    }
    await c.finishOnboarding(Profile(
      dailyMl: _goal,
      wakeHour: _wake,
      sleepHour: _sleep,
                  ...
                  child: Row(
                    children: [
                      if (idx > 0 && _step != _Step.building)
                        IconButton(
                            tooltip: 'Back', onPressed: _back, icon: const Icon(Icons.arrow_back_rounded))
                      else
                        const SizedBox(width: 48),
                      Expanded(child: _Dots(count: _steps.length, index: idx)),
                      const SizedBox(width: 48),
                    ],
                  ),
                ),
  ...
  Widget _frame({required String title, String? subtitle, required Widget body, Widget? footer}) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          if (subtitle != null) ...[
            ...
            Text(subtitle, style: const TextStyle(fontSize: 16, color: NaglyColors.textSecondary, fontWeight: FontWeight.w700)),
          ],
  ...
  Widget _continue({String label = 'Continue', VoidCallback? onPressed, bool enabled = true}) =>
      FilledButton(onPressed: enabled ? (onPressed ?? _next) : null, child: Text(label));
      ...
      body: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              _RoundIcon(icon: Icons.remove_rounded, label: 'Decrease weight', onTap: () => change(-1)),
              ...
              const SizedBox(width: 20),
              _RoundIcon(icon: Icons.add_rounded, label: 'Increase weight', onTap: () => change(1)),
            ],
          ),
          ...
          SegmentedButton<bool>(
            segments: const [ButtonSegment(value: false, label: Text('kg')), ButtonSegment(value: true, label: Text('lb'))],
            selected: {_lb},
  ...
  Widget _activityStep() {
    const options = [
      ...
      (ActivityLevel.veryActive, '🔥', 'Very active', 'Athlete / physical work'),
    ];
    return _frame(
  ...
  Widget _hoursStep() {
    ...
    return _frame(
      ...
      body: Column(
        children: [
          _HourRow(emoji: '☀️', label: 'Wake', hour: _wake, onChanged: (h) => setState(() => _wake = h)),
          const SizedBox(height: 12),
          _HourRow(emoji: '🌙', label: 'Sleep', hour: _sleep, onChanged: (h) => setState(() => _sleep = h)),
          const SizedBox(height: 28),
          Text('$waking', style: Theme.of(context).textTheme.displaySmall),
          const Text('waking hours', style: TextStyle(fontWeight: FontWeight.w800, color: NaglyColors.textSecondary)),
        ],
      ),
  ...
  Widget _buildingStep() => Center(
        child: Column(
          ...
          children: [
            const SizedBox(width: 64, height: 64, child: CircularProgressIndicator(strokeWidth: 6)),
            const SizedBox(height: 28),
            Text('Building your plan…', style: Theme.of(context).textTheme.headlineSmall),
            const SizedBox(height: 8),
            const Text('Mom is doing the math 👩',
        ...
        child: Column(
          ...
          children: [
            TweenAnimationBuilder<double>(
              ...
              builder: (context, v, _) =>
                  Text('${v.round()}', style: Theme.of(context).textTheme.displayLarge?.copyWith(fontSize: 84, color: NaglyColors.primaryDeep)),
            ),
            const Text('ml', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w900, color: NaglyColors.primaryDeep)),
            const SizedBox(height: 16),
            Text('Your daily goal', style: Theme.of(context).textTheme.titleLarge),
            const SizedBox(height: 6),
            Text('Based on $_weightKg kg · ${labels[_activity]} · ${waking}h awake',
                textAlign: TextAlign.center,
  ...
  Widget _medicationStep() => _frame(
          ...
          children: [
            TextField(
              ...
              decoration: InputDecoration(
                ...
                border: OutlineInputBorder(borderRadius: BorderRadius.circular(18), borderSide: const BorderSide(color: NaglyColors.outline)),
              ),
            ),
            const SizedBox(height: 12),
            NCard(
              onTap: () async {
                final t = await showTimePicker(context: context, initialTime: _medTime);
              ...
              child: Row(
                children: [
                  ...
                  const Expanded(child: Text('Remind me at', style: TextStyle(fontWeight: FontWeight.w800))),
                  Text(_medTime.format(context),
                      style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: NaglyColors.med)),
                ],
              ),
            ),
          ],
        ...
        footer: Column(
          ...
          children: [
            ListenableBuilder(
              listenable: _medName,
              builder: (_, _) => _continue(enabled: _medName.text.trim().isNotEmpty),
            ),
            ...
            AnimatedOpacity(
              ...
              child: Row(
                ...
                children: [
                  ...
                  const SizedBox(width: 12),
                  Expanded(child: SpeechBubble(text: '${_reaction ?? ''} 💛', name: _persona.displayName, tail: false)),
                ],
              ),
            ),
            const SizedBox(height: 16),
  ...
  Widget _permissionStep() => _frame(
        title: 'Let ${PersonaCatalog.relationship(_relationship).displayName} remind you?',
        body: Column(
          children: [
              ...
              children: [
                ...
                Expanded(
                  child: SpeechBubble(
                    ...
                    tail: false,
                    text: '${_persona.signature} Let me nudge you when you forget'
            ...
            const Spacer(),
            const Text('Reminders only arrive during your waking hours. You can log right from the notification.',
                textAlign: TextAlign.center, style: TextStyle(color: NaglyColors.textSecondary, fontWeight: FontWeight.w700)),
            const SizedBox(height: 16),
          ],
        ),
        footer: Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            _continue(label: 'Allow notifications', onPressed: () => _finish(askPermission: true)),
            TextButton(onPressed: () => _finish(askPermission: false), child: const Text('Maybe later')),
          ],
        ),
      );
}
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(title, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w900, color: NaglyColors.ink)),
                        const SizedBox(height: 2),
                        Text(subtitle, style: const TextStyle(fontWeight: FontWeight.w700, color: NaglyColors.textSecondary)),
                      ],
                    ),
                  ),
...
class _HourRow extends StatelessWidget {
  const _HourRow({required this.emoji, required this.label, required this.hour, required this.onChanged});
  ...
  Widget build(BuildContext context) => NCard(
        child: Row(
          children: [
            ...
            _RoundIcon(icon: Icons.remove_rounded, label: '$label earlier', onTap: () => onChanged((hour + 23) % 24)),
            SizedBox(
              width: 76,
              child: Text(formatHourLabel(hour),
                  textAlign: TextAlign.center,
                  style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: NaglyColors.ink)),
            ),
            _RoundIcon(icon: Icons.add_rounded, label: '$label later', onTap: () => onChanged((hour + 1) % 24)),
          ],
        ),
      );
}
