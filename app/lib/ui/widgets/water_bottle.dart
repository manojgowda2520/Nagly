...
class _WaterBottleState extends State<WaterBottle> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    if (widget.enableTilt) {
      try {
        _accel = accelerometerEventStream(samplingPeriod: SensorInterval.uiInterval).listen((e) {
                        ...
                        if (widget.label != null)
                          Text(widget.label!,
                              style: TextStyle(
                                ...
                                color: level > 0.45 ? Colors.white : NaglyColors.ink,
                                shadows: level > 0.45
                                    ? [const Shadow(color: Color(0x33000000), blurRadius: 8)]
                                    : null,
                              )),
                        if (widget.sublabel != null)
                          Text(widget.sublabel!,
                              style: TextStyle(
                                ...
                                color: level > 0.38 ? Colors.white.withValues(alpha: 0.9) : NaglyColors.textSecondary,
                              )),
                      ],
                    ),
                  ),
...
class _BottlePainter extends CustomPainter {
  ...
  Path _bottlePath(Size s) {
    ...
    return Path()
      ...
      ..cubicTo(w, neckH + shoulder * 0.2, w, shoulder * 0.9, w, shoulder + r * 0.2)
      ..lineTo(w, h - r)
      ..quadraticBezierTo(w, h, w - r, h)
      ..lineTo(r, h)
      ..quadraticBezierTo(0, h, 0, h - r)
      ..lineTo(0, shoulder + r * 0.2)
      ..cubicTo(0, shoulder * 0.9, 0, neckH + shoulder * 0.2, (w - neckW) / 2, neckH)
      ..close();
  }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final amp = 4 + 8 * slosh + 6 * (1 - splash) * (splash > 0 && splash < 1 ? 1 : 0);
    ...
    Path wave(double offset, double ampScale) {
      ...
      for (double x = -20; x <= size.width + 20; x += 4) {
        ...
        final y = surfaceY + tiltY + sin(x / size.width * 2 * pi + phase + offset) * amp * ampScale;
        p.lineTo(x, y);
      }
      return p
    ...
    if (level > 0.001) {
      canvas.drawPath(wave(pi, 0.8), Paint()..color = NaglyColors.primary.withValues(alpha: 0.45));
      canvas.drawPath(
        wave(0, 1),
        Paint()
          ..shader = const LinearGradient(
      ...
      if (splash > 0 && splash < 1) {
        final bubblePaint = Paint()..color = Colors.white.withValues(alpha: 0.7 * (1 - splash));
        for (final b in bubbles) {
          final t = ((splash - b.delay) / (1 - b.delay)).clamp(0.0, 1.0) * b.speed;
          final y = size.height - (size.height - surfaceY) * t;
          canvas.drawCircle(Offset(b.x * size.width + sin(t * 10) * 3, y), b.size, bubblePaint);
        }
      }
    }
    canvas.restore();
    ...
    canvas.drawPath(
      Path()
        ..addRRect(RRect.fromRectAndRadius(
            Rect.fromLTWH(size.width * 0.12, size.height * 0.3, size.width * 0.07, size.height * 0.45),
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(Rect.fromLTWH((size.width - capW) / 2, -size.height * 0.05, capW, size.height * 0.07),
